    ...
    <Box sx={{ width: '100vw', bgcolor: 'background.default', py: 10, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <Container maxWidth="xl" sx={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <Grid container columns={12} spacing={4} justifyContent="center" alignItems="center">
          {features.map((f, i) => (
            <Grid sx={{ gridColumn: 'span 4', display: 'flex', justifyContent: 'center' }} key={f.title}>
              <motion.div
              ...
              >
                <Paper elevation={4} sx={{ p: 4, borderRadius: 4, minHeight: 200, bgcolor: 'background.paper', color: 'text.primary', display: 'flex', flexDirection: 'column', alignItems: 'center', borderTop: `4px solid ${i % 2 === 0 ? '#246bfd' : '#7c3aed'}` }}>
                  {f.icon}
